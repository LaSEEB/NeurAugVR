function EEG = prep7(EEG,resamp,hp,lp,dirs,elims)
% E.g.:
% resamp = 250
% hp = 1
% lp = 40
...
prep_report.('trialsL') = sum(strcmp({EEGtemp.event(:).type}, 'L')) - sum(strcmp({EEG.event(:).type}, 'L'));
prep_report.('trialsR') = sum(strcmp({EEGtemp.event(:).type}, 'R')) - sum(strcmp({EEG.event(:).type}, 'R'));
