...
%             
%             [~, ind_01] = min(abs(f-0.1));
%             line([ f_hz; f_hz ], [ 0 max(FFTData(ind_01:end))+5 ], 'Color', 'red', 'LineWidth', 1);
%             line([ f_hz_bkg; f_hz_bkg ], [ 0 max(FFTData(ind_01:end))+5], 'Color', 'black', 'LineWidth', 1);
%             ylim([0 max(FFTData(ind_01:end))+5])
%             ylabel('Power [uV²]')
%             xlabel('Frequency [Hz]')
           
        end
    end
  %  title([dataset.chanlocs(n).labels, ' - f [Hz] = ', num2str(round(f(real_harmonics(n,:)), 2))])       
  % print(['/home/iesteves/MR_Correction_Pipeline/Pipeline/BCG_Artefact_Correction-master/Figures/resting_S006_harmonics_',dataset.chanlocs(n).labels], '-dpng')
end
